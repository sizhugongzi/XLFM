...
class LBFMPlayDetailController: UIViewController {
    ...
    private lazy var leftBarButton: UIButton = {
        ...
        button.frame = CGRect(x: 20, y: isIphoneX ? 44 : 20, width: 44, height: 44)
        button.setImage(UIImage(named: "playpage_icon_down_black_30x30_"), for: UIControl.State.normal)
        button.addTarget(self, action: #selector(letfBarButtonClick), for: UIControl.Event.touchUpInside)
        return button
    }()
